...
bool custom_graph::create_graph(std::string xml)
{
    ...
    // Source_Graph g;
        BOOST_FOREACH(ptree::value_type &v,pt.get_child("root.components"))
        {
            ...
            {
                local_graph[vt].type=Component_Type::NOT_SPECIFIED;
            }
            ...
            if(v.second.get_child_optional("direction"))
            {
                local_graph[vt].direction = v.second.get_child("direction").get_value<int>();
            }
        ...
        BOOST_FOREACH(ptree::value_type &v,pt.get_child("root.edges"))
        {
            ...
            from_port = (v.second.get_child_optional("from.port"))? v.second.get_child("from.port").get_value<int>() : 0;
            to_port = (v.second.get_child_optional("to.port"))? v.second.get_child("to.port").get_value<int>() : 0;
...
// DOES NOT HANDLES DOUBLE IN XML, creates a different vertex/edge.
  ...
  boost::write_graphviz(myfile, local_graph,make_vertex_writer(boost::get(&Custom_Vertex::layer, local_graph),boost::get (&Custom_Vertex::name, local_graph),boost::get(&Custom_Vertex::ports, local_graph), boost::get(&Custom_Vertex::type,local_graph ), boost::get(&Custom_Vertex::priority_category,local_graph))
      ,make_edge_writer(boost::get(&Custom_Edge::priority,local_graph),boost::get(&Custom_Edge::from_port,local_graph),boost::get(&Custom_Edge::to_port,local_graph))
